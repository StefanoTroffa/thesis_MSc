...
class MCMCSampler:
    def __init__(self, current_tuple, template=None,beta=None, graph=None, initialized=False, edge_pairs=None):
    ...
    def evaluate_model(self, model, graph_tuple):
        output= model(graph_tuple)
        amplitude, phase = output[0][0],output[0][1]
        return tf.complex(real=amplitude * tf.cos(phase),
                        imag=amplitude * tf.sin(phase))

    def time_evoluted_config_amplitude(self, model, graph_tuple):
    ...
    @tf.function()
    def ite_step(self, new_graphs, all_amplitudes, initial_gt,model):
        baseline_output = self.evaluate_model(model,initial_gt)  # Ensure this takes a single graph

        # Evaluate the model on all new graphs in a single batch operation
        model_outputs =model(new_graphs)  # shape: [batch_size, 2]
    ...
    @tf.function()
    def time_evoluted_config_amplitude_tf(self, graph_tuple, model, j2: float = 0.0):
        ...
        return self.ite_step(new_graphs, all_amplitudes, graph_tuple,model)
    def monte_carlo_update(self, N_sweeps, graph_tuple, approach,model):
        state = graph_tuple
        if approach == 'var':
            psi = self.evaluate_model(state,model)
        else:
            psi = self.time_evoluted_config_amplitude(state,model)
        for sweep_idx in range(N_sweeps):
            proposed_graph_tuple = propose_graph_tuple(state)
            if approach == 'var':
                psi_new = self.evaluate_model(proposed_graph_tuple,model)
            else:
                psi_new = self.time_evoluted_config_amplitude(proposed_graph_tuple,model)
    ...
    @staticmethod
    @tf.function(jit_compile=True)
    def calculate_acceptance_prob_w_log(psi_old: tf.Tensor, psi_new: tf.Tensor) -> tf.Tensor:
        """Vectorized acceptance probability calculation"""
        ratios = (tf.math.log(psi_new[:, 0]**2) /tf.math.log(psi_old[:, 0]**2) )  # Amplitude ratios
        return tf.minimum(1.0, tf.square(ratios))
    ...
    """
    @tf.function()  
    def monte_carlo_update_on_batch(self, model, GT_batch: GraphsTuple) -> Tuple[GraphsTuple, tf.Tensor]:
    ...
    def monte_carlo_update_on_batchv2(self,model, GT_batch: GraphsTuple) -> Tuple[GraphsTuple, tf.Tensor]:
        ...
        return current_batch, current_psi
@tf.function()
def compute_phi_terms(batched_graphs: GraphsTuple, sampler: MCMCSampler, model:snt.Module):
